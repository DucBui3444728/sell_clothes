...
// Generate 24 products for pagination testing
export const ALL_PRODUCTS: Product[] = Array.from({ length: 24 }).map((_, index) => {
    const baseProduct = baseProducts[index % baseProducts.length];
    return {
        ...baseProduct,
        id: `${index + 1}`,
        name: `${baseProduct.name} ${index >= 8 ? `V${Math.floor(index / 8) + 1}` : ''}`.trim()
    };
});
